...
export const getRecommendations = async (query?: RecommendationGetQueryInterface) => {
  const response = await axios.get(`/api/recommendations${query ? `?${queryString.stringify(query)}` : ''}`);
