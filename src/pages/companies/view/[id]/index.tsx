              ...
              {hasAccess('job_posting', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/job-postings/create?company_id=${data?.id}`}>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.job_posting?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/job-postings/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
                            ...
                            <Td>
                              {hasAccess('job_posting', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/job-postings/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('job_posting', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    job_postingHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
