  ...
  const { data, error, isLoading, mutate } = useSWR<JobPostingInterface>(
    ...
    () =>
      getJobPostingById(id, {
        relations: ['company', 'candidate', 'recommendation'],
      }),
  );
              ...
              {hasAccess('candidate', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/candidates/create?job_posting_id=${data?.id}`}>
              ...
              {hasAccess('recommendation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/recommendations/create?job_posting_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.recommendation?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/recommendations/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('recommendation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/recommendations/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('recommendation', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    recommendationHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
